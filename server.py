...
import server_report.create_response_header as create_response_header
...
def close_connection(conn, timeout=False, config=None):
    ...
    if timeout:
        ...
        conn.send(create_response_header.server_reply(config, report))
    #sys.stdout.write("Going to close connection\n")
    #sys.stdout.write("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n")
    if timeout:
        report = {}
        ...
        try:
            ...
            if data:
                ...
                for header in response_header:
                    ...
                    server_response = create_response_header.handle_server_response(config, server_report)
                    if server_response:
