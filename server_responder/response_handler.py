...
def handle_server_request(config, report):
    try:
        ...
        report = check_file_path(report, config)
           #Dummy 
            #config_encoding[] = configreader.read_accept_encoding_config_file()
...
def check_file_path(report, config):
    if os.path.exists(report["request"]["path"]):
        ...
        report = check_if_range_header(report, config)
        report = check_range_request(report, config)
        ...
        #report = accept_header(report, config)
    else:
...
"""
def check_file_redirects(report, config=None):
    try:
        # Check 301 redirects
