...
import response.dynamic_pages as dynamic_pages
import utility_functions

def create_header_response(config, report):
    try:
        ...
        now = utility_functions.change_datetime_to_string(datetime.utcnow())
        if now:
        ...
        if "request" in  report and report["request"]:
            if report["response"]["status_code"] == "200":
                if report["request"]["method"] == "OPTIONS":
                    ...
                    mime_response = rejoin_mime_type(config, report)
                ...
                report["response"]["payload"] = dynamic_pages.create_error_page(report)
            if report["request"]["Connection"]:
                report["response"]["Connection"] = report["request"]["Connection"]
        sys.stdout.write(f'Report\n{report}\n') 
        return report
    except Exception as e:
        sys.stderr.write(f'create_header_response: error {e}\n')

def rejoin_mime_type(config, report):
    try:
        mime_response = {}
        ...
        if file_path is None:
            ...
            mime_response["mime_type"] = config["HEADERS"]["mime_types"][1]
            mime_response["payload"] = dynamic_pages.create_directory_list(report, config)      
        else:
            with open(file_path, "rb") as fobj:
                mime_response["payload"] = fobj.read()
            mime_response["file_length"] = len(mime_response["payload"])
            mime_response["ETag"] = '"' + utility_functions.change_to_etag(mime_response["payload"]) + '"'
            last_modified = utility_functions.obtain_file_last_modified_time(file_path)
            if last_modified:
        ...
        sys.stderr.write(f'rejoin_mime_type: error {e}\n')
