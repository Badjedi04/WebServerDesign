...
def create_html_page(title="", heading="", body=""):
    html_content = """
        <!DOCTYPE html>
        <html>
            <head>
...
def create_error_page(report):
    ...
    return create_html_page(title, heading, body)

def create_directory_list(report, config):
    directory_listing = [f for f in os.listdir(report["request"]["path"])]

    table_rows = ""
    for i in range(0, len(directory_listing)):
        modified_time = utility_functions.obtain_file_last_modified_time(report["request"]["path"])
    ...
    """
    dir_listing_table = Environment().from_string(html_table).render(table_rows=table_rows)
    payload = create_html_page("Directory Listing", "Index of " + report["request"]["path"] + " :", dir_listing_table)
    return payload
